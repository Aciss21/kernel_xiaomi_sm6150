...
/*
 * sched_asym_cpucap_active - Asymmetric CPU capacity check for POC
 *
 * Upstream POC gates the fast path on symmetric CPU capacity because it
 * replaces the energy-aware idle scan.  This kernel (CAF 4.14, atoll)
 * runs WALT/EAS placement (find_energy_efficient_cpu) before
 * select_idle_sibling(), so POC only accelerates the plain wake fast
 * path.  The upstream gate is therefore intentionally disabled here to
 * let POC engage on asym-capacity devices.
 */
static inline bool sched_asym_cpucap_active(void)
{
	return false;
}
...
#ifdef CONFIG_SCHED_SMT
static inline bool sched_smt_active(void)
{
