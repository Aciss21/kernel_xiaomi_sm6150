...
/*
 * CAF 4.14.atoll backport notes (firelzrd poc-selector v2.6.3):
 *   - sched_ext hunk dropped (not present in 4.14); scx guards are inert.
 *   - sched_asym_cpucap_active() always false: WALT/EAS placement
 *     (find_energy_efficient_cpu) still runs before select_idle_sibling(),
 *     so POC only accelerates the plain wake fast path.
 *   - p->recent_used_cpu does not exist here; select_idle_cpu_poc is
 *     always called with recent = -1 (Levels 1r/4r inactive).
 *   - no CONFIG_SCHED_CLUSTER/cpu_clustergroup_mask(): cluster levels
 *     (2/5) are inert via the always-false sched_cluster_active key.
 *   - no sched_idle_cpu()/available_idle_cpu(): idle checks are handled
 *     by the CPU bitmap updated in do_idle(); SIS_UTIL overload gate
 *     mapped to SIS_PROP + nr_idle_scan (initialized to LLC span weight).
 */
...
/*
 * Cluster active: sched_cluster_active
 *
 * CAF 4.14 has no CONFIG_SCHED_CLUSTER / cpu_clustergroup_mask(), so the
 * L2-cluster Level 2/5 fast path is compiled into an inert always-false
 * static key here (declared in sched.h).  Only the LLC-wide Levels 1-6
 * run on this kernel.
 */
	...
	if (sched_smt_active()) {
		...
		if (core_mask) {
			...
			if (!static_branch_likely(&sched_poc_greedy_search) &&
			    sched_feat(SIS_PROP) && !READ_ONCE(sd_share->nr_idle_scan))
				return -2;
...
#ifdef CONFIG_SYSCTL
static int sched_poc_sysctl_zero;
static int sched_poc_sysctl_one = 1;
				    ...
				    void __user *buffer, size_t *lenp, loff_t *ppos)
{
	...
	struct ctl_table tmp = {
		.data    = &val,
		.maxlen  = sizeof(val),
		.extra1  = &sched_poc_sysctl_zero,
		.extra2  = &sched_poc_sysctl_one,
	};
...
static int sched_poc_rr_improved_sysctl_handler(struct ctl_table *table,
					     int write, void __user *buffer,
					     size_t *lenp, loff_t *ppos)
{
