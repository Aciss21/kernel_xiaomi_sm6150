...
#ifdef CONFIG_DRM
	cd->msm_drm_notifier.notifier_call = goodix_ts_msm_drm_notifier_callback;
	ret = msm_drm_register_client(&cd->msm_drm_notifier);
	if (ret)
		ts_err("Failed to register drm notifier client:%d", ret);
#endif
