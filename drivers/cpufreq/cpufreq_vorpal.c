 ...
 */
/*
 * Linux 4.14 has no thermal-pressure infrastructure (arch_scale_thermal_pressure
 * does not exist), so the LMH-style channel this reads on GKI 5.10 is absent.
 * The v2.2 thermal emergency net (rfx_thermal_fn via thermal_zone / temp_mc)
 * covers real die temperature instead, so pressure is hardwired to none here.
 */
static inline unsigned long rfx_thermal_pressure(int cpu)
{
	return 0;
}
					     ...
					     unsigned long max_cap)
{
	...
	press = rfx_thermal_pressure(cpumask_first(pol->related_cpus));
	if (max_cap) {
		if (press >= max_cap)
			press_pct = 0;
		else if (press)
...
static bool rfx_should_update_freq(struct rfx_policy *p, u64 time)
{
	...
	if (unlikely(!p || !p->policy))
		return false;
	if (cpumask_first(p->policy->cpus) != smp_processor_id())
		return false;
